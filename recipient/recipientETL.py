# author: alan hamm(pqn7) with AI assisted programming( parse_dates(), get_unique_vals() functions )
# date: october 2024

#%%
from recipient import get_unique_vals, parse_dates
import pandas as pd
...
#from recipient import get_unique_vals
#get_unique_vals(survey) # parameter is a dataframe


#%%
# define non-categorical dtypes
NON_CATEGORY_DTYPE = {
    'WRESPID': int,
    'DATSTART': str, # "%Y-%m-%d %H:%M:%S"
    'DATEND': str, # "%Y-%m-%d %H:%M:%S"
...
def read_csi(survey_csv):
    # Get the list of all column names
    column_names = pd.read_csv(survey_csv, nrows=0).columns.tolist()

    # Update dictionary to set remaining columns to 'category'
    ...
    survey = pd.read_csv(survey_csv, usecols=USE_COLS, dtype=NON_CATEGORY_DTYPE)
    ...
    for col in ['DATSTART', 'DATEND', 'ISSUED', 'PP_START', 'PP_END']:
        # survey[col] = survey[col].apply(lambda x: parse_dates(x) if not pd.isnull(x) else x)
...
#column_data_types = read_csi().dtypes
